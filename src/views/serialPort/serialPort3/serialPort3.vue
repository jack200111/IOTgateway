...
<template>
  <div>
    <h1>{{ title }}</h1>
    <div class="content">
      <span v-for="(item, index) in serialPort3" :key="index">
    ...
    <div class="content">
      <span v-for="(item, index) in serialPort3" :key="index+101">
        <p v-if="item.type !== 'button' && item.prop === '工作方式'">
          <!-- 下拉框 -->
          <span>
            <span class="prop">{{ item.prop }}:</span>
    ...
    <!-- 按钮 -->
    <span v-for="item in serialPort3" :key="item.prop">
...
export default {
  ...
  mounted () {
    this.fetchData('serialPort3')
  },
  methods: {
    async save () {
      await http.post('/serialPost3', { UART3: this.serialPort3, title: this.title })
    },
