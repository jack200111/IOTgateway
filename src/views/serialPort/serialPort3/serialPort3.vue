  ...
  methods: {
    async save () {
      if (confirm('设备重启生效是否继续')) {
        const UART3 = [...this.SelectArr, this.SelectProp[0], this.ModbusTCP, this.inputBaudrate, this.inputPORT]
        const res = await http.post('/serialPost3', { UART3 })
        console.log(res)
      }
    },
    fetchData () {
      http.get('/serialPort3').then(res => {
        console.log(res.data.UART3)
        Object.keys(res.data.UART3).forEach((item) => {
          this.SelectArr.forEach((item2) => {
            if (item === item2.label) {
              console.log(item2)
              item2.selected = res.data.UART3[item]
            }
          })
          this.SelectProp.forEach((item2) => {
            ...
            if (item2.label === 'ModbusTCP') {
              this.ModbusTCP.selected = res.data.UART3[item]
              if (res.data.UART3[item] === 'NO') {
                this.SelectProp[1].selected = 'None'
              } else if (res.data.UART3[item] === 'OFF') {
                this.SelectProp[1].selected = 'None'
              }
            }
          })
          if (item === this.inputBaudrate.label) {
            this.inputBaudrate.selected = res.data.UART3[item]
          }
          if (item === this.inputPORT.label) {
            this.inputPORT.selected = res.data.UART3[item]
          }
        })
      })
    },
