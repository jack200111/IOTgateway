...
<template>
  <div>
    <h1>{{ title }}</h1>
    <div class="content">
      <span v-for="(item, index) in serialPort1" :key="index">
    ...
    <div class="content">
      <span v-for="(item, index) in serialPort1" :key="index+101">
        <p v-if="item.type !== 'button' && item.prop === '工作方式'">
          <!-- 下拉框 -->
          <span>
            <span class="prop">{{ item.prop }}:</span>
    ...
    <!-- 按钮 -->
    <span v-for="item in serialPort1" :key="item.prop">
...
export default {
  ...
  mounted () {
    this.fetchData('serialPort1')
  },
  methods: {
    async save () {
      await http.post('/serialPost1', { UART1: this.serialPort1, title: this.title })
    },
